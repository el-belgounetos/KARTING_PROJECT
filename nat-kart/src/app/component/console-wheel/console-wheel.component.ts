...
export class ConsoleWheelComponent implements OnInit {
  ...
  initializeCupsImages() {
    this.allCupsImages = this.consolesDisplayed.map(() => 'http://localhost:8080/images/ui/intero.png');
  }
    ...
    this.consolesDisplayed.forEach((console, index) => {
      setTimeout(() => {
        if (console.cups && console.cups.length > 0) {
          ...
          this.allCupsImages[index] = 'http://localhost:8080/images/cups/' + console.name + '/' + console.cups[randomCupIndex].picture;
        }
